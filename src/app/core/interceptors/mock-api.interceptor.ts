...
export const mockApiInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (req.method === 'POST' && url === '/api/books') {
    ...
    const bookData = req.body as any;
    const newBook = {
      id: `book_${Date.now()}`,
      ...bookData,
  ...
  if (req.method === 'DELETE' && url.startsWith('/api/books/')) {
    ...
    if (hasActiveLoans) {
      return throwError(() => new HttpErrorResponse({
        status: 409,
        statusText: 'Conflict',
        error: { message: 'Impossible de supprimer un livre actuellement emprunté' }
      })).pipe(delay(randomDelay));
    }
  ...
  if (req.method === 'POST' && url === '/api/loans') {
    ...
    if (activeUserLoans >= 3) {
      return throwError(() => new HttpErrorResponse({
        status: 409,
        statusText: 'Conflict',
        error: { message: 'Limite d\'emprunts simultanés atteinte (3 maximum)' }
      })).pipe(delay(randomDelay));
    }
  ...
  if (req.method === 'PUT' && url.includes('/api/loans/') && url.endsWith('/return')) {
  ...
  if (req.method === 'PUT' && url.includes('/api/loans/') && url.endsWith('/renew')) {
    const loanId = url.split('/')[3];
    const loan = mockDb.loans.find(l => l.id === loanId);
    ...
    if (loan.status !== LoanStatus.ACTIVE) {
      return throwError(() => new HttpErrorResponse({
        status: 409,
        statusText: 'Conflict',
        error: { message: 'Seuls les emprunts actifs peuvent être renouvelés' }
      })).pipe(delay(randomDelay));
    }
    ...
    if (loan.renewalCount >= 2) {
      return throwError(() => new HttpErrorResponse({
        status: 409,
        statusText: 'Conflict',
        error: { message: 'Limite de renouvellements atteinte (2 maximum)' }
      })).pipe(delay(randomDelay));
    }
